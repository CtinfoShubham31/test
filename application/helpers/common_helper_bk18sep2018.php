...
//                    echo '<option selected value="'.$label_data->label_id.'" style="padding-left:'.$sub_mark.'px">'.$label_data->label_name.'</option>';
//                }else{
            ...
            if(!empty($users_array)){
                ...
                if(!empty($get_co_own)){
                    foreach($get_co_own AS $cown){
                        $string .= '<span class="co-owner-box"> '.$cown->first_name.' '.$cown->last_name.' </span>';
                    }
                }
            }
        ...
        if(!empty($tree_label)){ 
           // $strg .= '<ul class="cd-accordion-menu animated">';
            foreach($tree_label AS $tlab){
                ...
                <label for="group-'.$tlab->label_id.'" class="label-title relative">
                    <span>'.$tlab->label_name.'</span> <div class="label-search-icon"><i class="fa fa-search"></i><i class="fa fa-search" onclick="searchLabel('."'".$tlab->label_name."'".')"></i></div>'; 
            ...
            foreach($tree_label AS $tlab){
                ...
                if(!empty($label_data2)){
                    ...
                    if($tlab->label_creator_id == $user_id){
                        $strg .= '<div class="edit-tag"><span data-toggle="modal" data-target="#manage_label"></span><i class="fa fa-random" onclick="mergeLabel('.$tlab->label_id.')" data-toggle="modal" data-target="#edit_label" aria-hidden="true"></i><i data-toggle="modal" data-target="#edit_label" onclick="editManageLabel('.$tlab->label_id.')" class="fa fa-pencil" data-original-title="Edit"></i>
                            <i class="fa fa-trash" data-toggle="tooltip" data-placement="top" title="" data-original-title="Delete" onclick="delete_label('.$tlab->label_id.')"></i></div>';
                    }
                    else if(!empty($usrdata) && $usrdata->is_super_user){
                        $strg .= '<div class="edit-tag"><span data-toggle="modal" data-target="#manage_label"></span><i class="fa fa-random" onclick="mergeLabel('.$tlab->label_id.')" data-toggle="modal" data-target="#edit_label" aria-hidden="true"></i>
                            </div>';
                    }
                    
                }else{
                    if($tlab->label_creator_id == $user_id){
                        $strg .= '<div class="edit-tag edit-tag2"><span data-toggle="modal" data-target="#manage_label"></span><i class="fa fa-random" onclick="mergeLabel('.$tlab->label_id.')" data-toggle="modal" data-target="#edit_label" aria-hidden="true"></i><i data-placement="top" title="" class="fa fa-pencil" data-original-title="Edit" data-toggle="modal" data-target="#edit_label" onclick="editManageLabel('.$tlab->label_id.')"></i><i class="fa fa-trash" onclick="delete_label('.$tlab->label_id.')" data-toggle="tooltip" data-placement="top" title="" data-original-title="Delete"></i></div>';
                    }
                    else if(!empty($usrdata) && $usrdata->is_super_user){
                        $strg .= '<div class="edit-tag"><span data-toggle="modal" data-target="#manage_label"></span><i class="fa fa-random" onclick="mergeLabel('.$tlab->label_id.')" data-toggle="modal" data-target="#edit_label" aria-hidden="true"></i>
                            </div>';
...
if(!function_exists('getPvhScore')){
    function getPvhScore(){
        ...
        $posthelpfulscore_Data = $CI->common_model->getPostHelpfulScore($where_arra);
        $posthelpfulscore_count = !empty($posthelpfulscore_Data) ? COUNT($posthelpfulscore_Data) : 0;
        ...
        $string .= '<div class="widget count-post">
                    ...
                    <div class="col-xs-4 padding-none">
                        <div class="box-circle" align="center"> <span> '.$posthelpfulscore_count.' </span> 
                            <p> Helpful</p>
                        </div>
                    </div>
                </div>
            ...
            <form role="form" id="search_form" class="search-form" action="dashboard/user_dashboard/search_posts" method="post">
                <div class="col-md-10 col-sm-10">
                    ...
                    <div class="advanc-search">
                        <a href="javascript:;" data-toggle="modal" data-target="#add_label" onclick="advSearchPopup()"> 
...
//if(!function_exists('taggedLabels')){
//    function notMyTaggedLabels($label_id){
//        if($label_id){
//            
//            $CI = &get_instance();
//            //$user_id = $CI->session->userdata('kaseidon_user_id');
...
//            $label_name = '';
//            
//            $lab_arr = array(''.LABEL.'.label_creator_id !='=>$user_id);
//            $single_label_details = $CI->post_model->getTreeLabels($lab_arr);
//
//            $string = '';
//            if(!empty($single_label_details->label_parent_id)){
//                $string .= $single_label_details->label_name.'/';
...
//if(!function_exists('nestedLabelExistance')){
//    
...
//        if(!empty($result)){
//            foreach($result as $label_data){
//                $lab_ids .= $label_data->label_id.',';
//                nestedLabelExistance($label_data->label_id);
//            }
//        }
//        echo $lab_ids;
//        
