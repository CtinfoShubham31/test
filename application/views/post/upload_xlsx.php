    ...
    <tr id="theadrow">
        ...
        if(!empty($lists_data[0])){
            ...
            for($p = 0;$p <= COUNT($lists_data[0])-1;$p++){
                //echo $p.'xxx';
        ?>
            <th id="sheet<?php echo $alpha_arr[$p].'1';?>"><?php echo $lists_data[0][$p];?><span onclick="deleteTableCol('<?php echo $alpha_arr[$p];?>')"><i class="fa fa-trash table_del" data-original-title="Delete" style="cursor:pointer"></i></span>&nbsp;&nbsp;<span onclick="updateSheetCol('<?php echo $alpha_arr[$p].'1';?>','<?php echo $lists_data[0][$p];?>')"><i class="fa fa-pencil table_edit" data-original-title="Delete" style="cursor:pointer"></i></span></th>
        <?php
            }
        }
        ?>
