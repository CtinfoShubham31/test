...
?>
<div id="<?php echo $uposts->post_id;?>" class="blog-item" <?php echo $style_css;?>>
    <div class="row">
        <div class="col-xs-12 col-sm-12">
                <?php
                $post_class = '';
                if(strtotime($uposts->updated_date)!=FALSE && !in_array($this->session->userdata('kaseidon_user_id'),explode(",",$uposts->commented_views)) && $uposts->updated_date!='0000-00-00 00:00:00'){
                    $post_class= 'update';
                }else if(commentCount($uposts->post_id) && !in_array($this->session->userdata('kaseidon_user_id'),explode(",",$uposts->updated_views))){
                    $post_class= 'notify';
                }
                ?>
