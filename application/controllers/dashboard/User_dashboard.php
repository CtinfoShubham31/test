...
//            $data['userposts'] = $this->post_model->getPosts($post_array,$page,$sort_posts);
    ...
    public function followed_posts(){
        ...
        $data['userposts'] = $this->post_detail_model->getFollowedPosts($follow_array,'Multiple',$page,$sort_posts);
    ...
    public function recentviewed_posts(){
        ...
        $data['userposts'] = $this->post_detail_model->getViewedPosts($view_array,'Multiple',$page,$sort_posts);
...
//            }
    ...
    public function individual_search(){
        ...
        $data['ptab'] = $this->input->post('ptab');
        //print_r($_POST);die('EEE');
        $where_string = '';
        $user_result = $this->common_model->getIndividualUserDetails($user_id,$company_id);
        $page = $data['page'] = 1;
        
        //$where_array = array();
//        if(!empty($_POST['primary_own'])){
//            //array_push($where_array,"blue","yellow");
//            $search_data = array(''.POST.'.post_creator_id'=>$_POST['primary_own']);
//        }
//        else if(!empty($_POST['co_ownr'])){
//            $search_data = array(''.POST.'.post_creator_id'=>$_POST['primary_own']);
//        }
//        else if(!empty($_POST['lbdata'])){
//            $search_data = array(''.POST.'.post_creator_id'=>$_POST['primary_own']);
//        }
        //print_r($_POST);
        $where_search = '';
        if(!empty($_POST['primary_own'])){
            $where_search = " ".POST.".post_creator_id = ".$_POST['primary_own']." "; 
            //$this->db->where($where_search); 
        }
        else if(!empty($_POST['co_ownr'])){
            $where_search = "FIND_IN_SET('".$_POST['co_ownr']."', ".POST.".co_owners)";
            //$this->db->where($where_search); 
        }
        else if(!empty($_POST['lbdata'])){
            $label_data = $this->post_model->getIndividualLabelUsingName($_POST['lbdata']);
            //print_r($label_data);die;
            
            if(!empty($label_data)){
                $a=1;
                foreach($label_data AS $lbd){
                    if($a>1){
                        $where_search .= " OR ";
                    }
                    $where_search .= "FIND_IN_SET('".$lbd->label_id."', ".POST.".tagged_labels)"; 
                    $a++;
                }
            }
        ...
        //echo $where_search;die('ZZ');
        
        
        if(!empty($_POST['ptabb']) && $_POST['ptabb'] == 1){
            $data['all_posts'] = $view_data['all_posts'] = 1;
            if(!empty($user_result) && $user_result->is_super_user != 1){
                $where_array = array(''.POST.'.company_id'=>$company_id,''.POST.'.content_request_status!='=>1);
                $where_string = "(".POST.".publish != 1 OR ".POST.".post_creator_id = '".$user_id."')"; 
            ...
            $data['userposts'] = $this->search_model->searchIndividuallly($where_array,$where_string,$where_search,$page);
        }
        else if(!empty($_POST['ptabb']) && $_POST['ptabb'] == 2){
            $data['my_posts'] = $view_data['my_posts'] = 1;
            if(!empty($user_result) && $user_result->is_super_user != 1){
                $where_string = "(".POST.".post_creator_id = '".$user_id."' OR FIND_IN_SET('".$user_id."', ".POST.".co_owners))"; 
                $where_array = array(''.POST.'.company_id'=>$company_id,''.POST.'.content_request_status!='=>1);
        ...
        else if(!empty($_POST['ptabb']) && $_POST['ptabb'] == 3){
            $data['followed_posts'] = $view_data['followed_posts'] = 1;
            $where_array = array(''.POST.'.company_id'=>$company_id,''.POST.'.content_request_status!='=>1,''.FOLLOWED_POST.'.followed_by'=>$user_id);
            $data['userposts'] = $this->search_model->searchIndividualllyFollowedPosts($where_array,$where_string,$where_search,$page);
        }
        else if(!empty($_POST['ptabb']) && $_POST['ptabb'] == 4){
            $data['recentviewed_posts'] = $view_data['recentviewed_posts'] = 1;
            $where_array = array(''.POST.'.company_id'=>$company_id,''.POST.'.content_request_status!='=>1,''.VIEWED_POST.'.viewed_by'=>$user_id);
            $data['userposts'] = $this->search_model->searchIndividualllyRecentlyViewed($where_array,$where_string,$where_search,$page);
